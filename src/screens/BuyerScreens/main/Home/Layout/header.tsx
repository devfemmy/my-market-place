...
const UserHeader = ({name, image}: any) => {
  ...
  const buyerNotification = notification?.filter(
    (data: any) => !data?.isStore && data?.status === 'UNREAD',
  );
      ...
      {auth || token ? (
        <View style={styles.container}>
          ...
          <View style={styles.textCard}>
            <Text text={` Hi, ${name} 👋`} fontSize={hp(20)} color={colors.gray} />
          </View>
        </View>
      ) : (
        <View>
            ...
            <>
              <Pressable onPress={() => navigation.navigate('Notification')}>
                {buyerNotification?.length > 0 && (
                  <Badge size={15} style={styles.bg2}>
