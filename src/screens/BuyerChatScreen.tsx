...
const BuyerChatScreen = ({ navigation }: any) => {
      ...
      await dispatch(getProfile()).then(async (dd: any) => {
        ...
        firestore().collection('messaging').onSnapshot((documentSnapshot: any) => {
          ...
          documentSnapshot?.forEach((doc: any) => {
            if (doc.data().sender?.id === dd?.payload?.id || doc.data().receiver?.id === dd?.payload?.id) {
              var data = {
                messageId: doc.id,
                id: doc.data().sender?.id === userProfile?.id ? doc.data().receiver?.id : doc.data().receiver?.id === userProfile?.id ? doc.data().sender?.id : "",
                name: doc.data().sender?.id === userProfile?.id ? doc.data().receiver?.name : doc.data().receiver?.id === userProfile?.id ? doc.data().sender?.name : "",
                image: doc.data().sender?.id === userProfile?.id ? doc.data().receiver?.imageUrl : doc.data().receiver?.id === userProfile?.id ? doc.data().sender?.imageUrl : "",
              ...
              <Pressable onPress={() => routeChat(data)}>
                <View style={styles.container2} key={data?.id} >
                  ...
                  <View style={styles.div3}>
                    <Text text={`${moment(new Date(data?.createdAt?.seconds * 1000)).calendar()}`} fontSize={hp(10)} />
