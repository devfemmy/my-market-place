...
import {
  ...
  getProductBySlug,
  getProductVariantSpec,
  productBySlug,
  updateProductVariant,
  updateProductVariantSpec,
} from '../redux/slices/productSlice';
import {
  ProductColorAloneData,
  ProductColorData,
  ProductNoColorData,
  ProductSizeData,
} from '../utils/types';
...
import {
  ProductColorAloneSchema,
  ProductColorSchema,
  ProductNoColorSchema,
  ProductSizeSchema,
...
const ColorEditVariant = (props: any) => {
  ...
  const [productInDraft, setProductInDraft] = useState<any>(
    props?.route.params.params.productEditInDraft,
  );
  ...
  useEffect(() => {
    const loadAsyn = async () => {
      var productInDrafts = await AsyncStorage.getItem('productEditInDraft').then((req: any) =>
        JSON.parse(req),
      );
  ...
  useEffect(() => {
    const loadData = async () => {
      dispatch(getProductBySlug(slug)).then(dd => {
        var filterData = dd?.payload?.product_variants?.find(
          (dat: any) => dat?.id === editableItem,
        );
        ...
        <View style={styles.minidiv} key={i}>
          <View style={globalStyles.rowStart}>
            <Text
              ...
              fontSize={hp(14)}
              fontWeight="400"
              style={{marginTop: hp(10), marginRight: hp(10)}}
              color={colors.gray}
            />
            <Text
              text={data?.size}
  ...
  const {
    ...
    handleChange: colorHandleChange,
    handleSubmit: colorHandleSubmit,
    handleBlur: colorHandleBlur,
    resetForm: colorResetForm,
  } = useFormik({
    initialValues: init,
  ...
  return (
    <View style={styles.container}>
      <MobileHeader props={props} categoryName="Product Details" />
      <View style={styles.columnContainer}>
        <Text text="Upload Images" fontSize={hp(16)} fontWeight="400" />

        {multipleUpload?.length < 1 || multipleUpload === undefined ? (
          <ImageUploadComponent single profileImageChange={pickImage} imageLoader={imageLoader} />
        ) : (
          <View style={styles.rowStart}>
            ...
            {dummyUploadImage
              ?.filter((a, b) => multipleUpload?.length < b + 1 && multipleUpload?.length < 6)
              .map((data, i) => {
                return (
                  <Pressable onPress={() => pickImage(i)}>
                    <View style={styles.multContainer2}>
                      {imageLoader ? (
                        <AntDesign name="loading1" color={'white'} />
                      ) : (
            ...
            <ScrollView showsVerticalScrollIndicator={false} showsHorizontalScrollIndicator={false}>
              ...
              <Pressable onPress={showModal}>
                <View style={[globalStyles.rowStart, {marginVertical: hp(10)}]}>
                  <Image source={plusBig} style={styles.plus} />
                  <Text
                    text="Add Sizes"
      ...
      <View style={styles.bottomContainer}>
        {!productInDraft?.isColor && !productInDraft?.isSize && (
          <Button isLoading={loader} title={'Update'} onPress={handleSubmit} />
        )}
        {productInDraft?.isColor && productInDraft?.isSize && (
          <Button isLoading={loader} title={'Update'} onPress={_handleSubmit} />
        )}
        {!productInDraft?.isColor && productInDraft?.isSize && (
          <Button isLoading={loader} title={'Update'} onPress={sizeAloneSubmission} />
        )}
        {productInDraft?.isColor && !productInDraft?.isSize && (
          <Button isLoading={loader} title={'Update'} onPress={colorHandleSubmit} />
        )}
      </View>
      ...
      <RBSheet
        ...
        <View style={styles.sheet}>
          ...
          <View style={styles.columnContainer2}>
            ...
            <View style={globalStyles.rowStart}>
              <View style={styles.subdiv}>
                <Input label="Quantity" number value={modalQuantity?.toString()} />
              </View>
            ...
            <Button
              isLoading={loader}
              title={editSizeData ? 'Update' : 'Save and add new'}
